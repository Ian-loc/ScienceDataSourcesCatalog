#!/usr/bin/env python3
"""Validate the normalized Instance 1 pilot after promotion from staging."""
    ...
    with psycopg.connect(args.database_url) as connection:
        actual = {
            "sources": scalar(
                ...
                "WHERE stable_id IN ('DR0011','DR0019')",
            ),
            "families": scalar(
                connection,
                "SELECT count(*) FROM catalog.product_families "
                "WHERE stable_id IN ('PF000001','PF000002','PF000003','PF000004','PF000005')",
            ),
            "products": scalar(
                connection,
            ...
            "capabilities": scalar(
                ...
                JOIN catalog.distributions d
                  ON d.distribution_id = ac.distribution_id
                WHERE d.stable_id IN (
                    'DD000006','DD000016','DD000017','DD000018','DD000019'
                )
                """,
            ),
            "assertions": scalar(
                ...
                "AND entity_stable_id IN ('DP000005','DP000011')",
            ),
            "reviews": scalar(
                connection,
                "SELECT count(*) FROM catalog.curation_reviews "
                "WHERE entity_type = 'product' "
                "AND entity_stable_id IN ('DP000005','DP000011')",
            ),
            "migrated_product_rows": scalar(
                connection,
                "SELECT count(*) FROM staging.v_latest_products "
                "WHERE migration_status = 'migrated'",
            ),
            "mapped_product_rows": scalar(
                connection,
                "SELECT count(*) FROM staging.v_latest_products "
                "WHERE migration_status = 'mapped'",
            ),
            "migrated_distribution_rows": scalar(
                connection,
        ...
        prohibited = scalar(
            connection,
            "SELECT count(*) FROM catalog.products "
            "WHERE stable_id IN ('DP000007','DP000008','DP000009','DP000010')",
        )
        if prohibited:
            failures.append(
                f"{prohibited} serviço(s), catálogo(s) ou infraestrutura(s) "
                "foram promovidos indevidamente como produto"
            )

        incomplete_products = scalar(
            connection,
            """
            SELECT count(*)
        ...
        print(f"OK: piloto normalizado íntegro — {actual}")
        return 0
