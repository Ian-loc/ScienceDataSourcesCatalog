...
**Data/hora:** 2026-08-05 22:00–22:12, `America/Sao_Paulo`  
**Família:** `PF000001`  
**Pacote relacionado:** `PRODES-ASSET-AMAZON-GEOPACKAGE`

## Unidades auditadas

| Unidade | Produto candidato | UUID de metadado |
|---|---|---|
| Hidrografia | `PD-PRODES-AMZ-HYDROGRAPHY` | `1df78632-68e7-4e91-bca0-25305d3f831e` |
| Hidrografia em não floresta | `PD-PRODES-AMZ-NON-FOREST-HYDROGRAPHY` | `87fb6a32-01c1-4421-b7d0-a93568e1b079` |
| Máscara de não floresta | `PD-PRODES-AMZ-NON-FOREST-DOMAIN-MASK` | `bed1276c-aa3d-4f5b-b560-1879617ef13d` |

## Objetivo

Preservar a identidade de três componentes cartográficos auxiliares do PRODES Amazônia, evitando que sejam normalizados como supressão de vegetação, incrementos, taxas, resíduos, incerteza ou simples atributos herdados do mapa anual.
...
É candidata a produto de suporte e delimitação de domínio. Não representa supressão ocorrida, ausência de vegetação, ausência de dados nem área acumulada de desmatamento.

```text
não floresta
= classe relativa à definição de Floresta adotada no mapeamento
≠ área sem vegetação
≠ dado ausente
≠ supressão
```
...
- nome oficial estável no nível de produto;
...
**Evidência:** metadados oficiais distinguem hidrografia geral, hidrografia em não floresta e máscara de não floresta, com objetos, classes e recortes próprios.  
**Correção:** contrato e validador preservam três identidades, proíbem interpretações de supressão e bloqueiam herança automática de temporalidade, sensores, classes e release.  
**Teste:** `scripts/validate_prodes_amazon_ancillary_layers_guard.py`, integrado ao gate agregado do GeoPackage PRODES Amazônia.  
...
Os dez componentes declarados do GeoPackage PRODES Amazônia agora possuem fronteiras protegidas por gates especializados ou por contratos operacionais prévios. Este resultado não equivale à conclusão dos produtos: os endpoints e bytes continuam não resolvidos e nenhuma promoção foi autorizada.
